import { consumerWallet } from "./clients.js";
import { chacha20_poly1305_seal, ecdh } from "@solar-republic/neutrino";
import { concat, json_to_bytes } from "@blake.regalia/belt";
import { getGatewayEncryptionKey } from "./gateway.js";
import { makeSignDoc, isSecp256k1Pubkey, serializeSignDoc, makeStdTx, } from "@cosmjs/amino";
...
export const getEncryptedSignedMsg = async (signer, msg, gatewayKey) => {
    gatewayKey ?? (gatewayKey = await getGatewayEncryptionKey());
    ...
    const sharedKey = sha256(ecdh(consumerWallet.privateKey, gatewayKeyBytes));
    const payload = {
    ...
    const ciphertext = concat(chacha20_poly1305_seal(sharedKey, nonce, json_to_bytes(payload)));
...
export const getArb36SignData = (signerAddress, data) => ({
...
export const getArb36SignDoc = (signerAddress, data) => {
    const msg = getArb36SignData(signerAddress, data);
    return makeSignDoc([msg], { gas: "0", amount: [] }, "", "", 0, 0);
};
export const getArb36Credential = async (signer, data) => {
    const accounts = await signer.getAccounts();
    const firstAccount = accounts[0];
    const signerAddress = firstAccount.address;
...
export const verifyAdr36Tx = async (signed) => {
