...
import { codeConfigExists, contractConfigExists, ibcConfigExists, saveIbcConfig } from './config.js';
import { instantiateContracts, uploadContracts } from './contracts.js';
import { CONSUMER_CHAIN_ID, CONSUMER_TOKEN } from './env.js';
import { toHex, toUtf8 } from 'secretjs';
import { sha256 } from '@noble/hashes/sha256';
const getIbcConnection = async () => {
    const secretStates = (await secretClient.query.ibc_client.clientStates({})).client_states;
    for (const state of secretStates.reverse()) {
        try {
            const stateRes = state.client_state;
...
export const getIbcChannel = async (connection) => {
...
const setupIbc = async () => {
    ...
    if (!channel)
        return false;
    const ibc_denom = "ibc/" + toHex(sha256(toUtf8(channel.channel_id + '/transfer/' + CONSUMER_TOKEN))).toUpperCase();
    saveIbcConfig({
